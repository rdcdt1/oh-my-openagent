...
import { OPENFERENCE_ENV_VAR, OPENFERENCE_PROVIDER_ID } from "../features/openference-provider"
...
describe("config handler Openference provider injection", () => {
  ...
  beforeEach(async () => {
    mock.restore()
    tempDataDir = mkdtempSync(path.join(tmpdir(), "openference-config-handler-"))
    ...
    ;({ createConfigHandler } = await import(`./config-handler?openference=${Date.now()}-${Math.random()}`))
  })
  ...
  test("registers the provider before provider model limits are harvested", async () => {
    // given the Openference API key present and a config with no provider block
    ...
    expect(providers?.[OPENFERENCE_PROVIDER_ID]).toBeDefined()
    const cachedOpenferenceKeys = [...modelCacheState.modelContextLimitsCache.keys()].filter((key) =>
      key.startsWith(`${OPENFERENCE_PROVIDER_ID}/`),
    )
    expect(cachedOpenferenceKeys.length).toBeGreaterThanOrEqual(10)
  })

  test("leaves the provider block absent when no credential is available", async () => {
    // given no Openference credential in the environment
    delete process.env[OPENFERENCE_ENV_VAR]
    const modelCacheState = createModelCacheState()
    const config: Record<string, unknown> = { model: "anthropic/claude-opus-4-7", agent: {} }
    const handler = createConfigHandler({
    ...
    // then no openference provider or model limit is registered
    const providers = config.provider as Record<string, unknown> | undefined
    expect(providers?.[OPENFERENCE_PROVIDER_ID]).toBeUndefined()
    expect(
