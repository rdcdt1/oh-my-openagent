...
/**
 * Injects Openference into opencode's live config so its models appear without
 ...
 * `openference` entry in opencode's auth.json. Without one, the config is left
...
function hasCredential(): boolean {
  const apiKey = process.env[OPENFERENCE_ENV_VAR]
  if (typeof apiKey === "string" && apiKey.length > 0) {
    return true
  }

  return getProviderAuthType(OPENFERENCE_PROVIDER_ID) !== undefined
}
...
export function applyOpenferenceProviderConfig(config: Record<string, unknown>): void {
  if (!hasCredential()) {
    return
  }
  ...
  const existingProvider = providers[OPENFERENCE_PROVIDER_ID]
  const provider: Record<string, unknown> = isRecord(existingProvider) ? existingProvider : {}
  providers[OPENFERENCE_PROVIDER_ID] = provider

  fillMissing(provider, {
    name: PROVIDER_NAME,
    npm: PROVIDER_NPM,
    env: [OPENFERENCE_ENV_VAR],
    options: { baseURL: OPENFERENCE_BASE_URL },
  })
